...
const uploadMedia = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedMimeTypes.includes(file.mimetype)) {
      ...
      cb(new Error('Invalid file type! Allowed types: Images and Videos.'), false);  // Reject the file
    }
  },
});
    ...
    try {
      ...
      if (files.video) {
        response.videos = files.video.map((file) => ({
          ...
          uniqueId: file.filename,  // This is the unique file name generated by uuid

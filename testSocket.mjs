...
// 🔧 Server config
const SERVER_URL = "http://localhost:2007";
const userId = "68062193600f323429cb2779"; // replace with test user ID
...
socket.on("connect", () => {
    ...
    // 🧪 Test deleteMsg emit after connect
    setTimeout(() => {
        socket.emit("deleteMsg", {
            chatKey: "chat:68062193600f323429cb2779:6837ee15063873b81e61eb4d", // your Redis key
            chatId: "684c55be2f1af82c1fef7146", // MongoDB document _id
            messagesToDelete: [
                {
                    _id: "684d09e38a563b7f8d3e488a" // ID of the message to delete
                }
            ]
        });
    }, 2000);
...
// 📥 Listen for response
socket.on("deletedMsg", (res) => {
    console.log("🗑️ Deleted Message Response:", res);
});
